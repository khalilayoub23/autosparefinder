...
| 2026-08-13 | **The owner reported NOA's broken Hebrew for the FOURTH time. Three previous fixes had all narrowed a regex inside the same architecture — and the architecture was the bug.** Measured on hand-written CORRECT Hebrew: `"הרפידות הכי זול בארץ"`→`"הרפידות מחירים תחרותיים"`, `ה-Bosch`→`ה Bosch` (stranded letter), a disclosure glued onto the closing question, and an ordinary `(2)` quantity causing a whole priced sentence to be **deleted** | The pipeline was `generate → REWRITE the text with regexes → publish`. A regex cannot see the grammar around the span it replaces, so every compliance/quality "fix" was a coin flip on the sentence. Two of the four defects were the SAME false-positive family already logged on 2026-07-20 (`מ-198`), fixed for the one reported form and never for the class: the lone-letter check still broke on the gershayim in `מק"ט`/`ק"מ`/`ש"ח` — the commonest term in a parts post — sending every such post to a repair path that stapled boilerplate onto it | **A post-processor may VALIDATE, and may add or drop a WHOLE LINE. It may never rewrite words inside a sentence.** A draft that fails is REGENERATED with its defects named back to the writer (`write_post`), or held for the owner — never patched, and never replaced by canned filler (a template post IS the bot voice). New `social/hebrew_style.py` is the deterministic FORM gate, the counterpart to `post_guard.py`'s MEANING gate, calibrated against real correct posts BEFORE being allowed to reject anything. Also: the fixed post skeleton (hook→wink→fact→price→CTA→question, twice a day forever) reads as generated even when every sentence is correct — 8 rotating post FORMS now vary what is present and in what order. **When a defect class is found, fix the CLASS: grep for every other input shape it can fire on, not just the one that was reported.** |
...
**Never regress (NOA language, 2026-08-13):** NOA's post-processor is **non-mutating**. It may
validate, and it may add or drop a WHOLE LINE (hashtag line, link line, the TikTok conditions
line) — it may **never** rewrite words inside a sentence. Compliance patterns live in
`_NOA_RISKY_CLAIM_RULES` as pattern+guidance and are REPORTED, never substituted. Language
defects are reported by `social/hebrew_style.py` (the FORM gate, counterpart to
`social/post_guard.py`'s MEANING gate). A failing draft goes back through
`SocialMediaManagerAgent.write_post()` to be **rewritten**, and if it still fails we publish
NOTHING and tell the owner — there are no canned fallback captions any more, and every deleted
one is listed in `hebrew_style.BOILERPLATE` so it cannot return. Any new language/quality check
must first be proven SILENT on the correct posts in `devtests/noa_language_test.py` before it is
allowed to reject anything, and must handle Hebrew abbreviations (`מק"ט`, `ק"מ`, `ש"ח`) and
maqaf-bound proclitics (`מ-198`, `ב-2020`, `ה-Bosch`) — those two forms have now caused this
same class of bug twice. Post SHAPE rotates via `_POST_FORMS`: never collapse it back to one
fixed skeleton, or the posts read as generated no matter how good the Hebrew is.
