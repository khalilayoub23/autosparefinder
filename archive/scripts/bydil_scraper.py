#!/usr/bin/env python3
"""
BYD Israel (bydauto.co.il) parts catalog scraper + DB importer.

Official Israeli BYD importer: Shlomo Motors.
API: POST https://bydauto.co.il/wp-admin/admin-ajax.php
     action=check_mehiron_action  cnumber=<exact_sku>  cdesc=<contains_3+>
...
async def import_to_db(parts: list[dict], dry_run: bool = False) -> dict:
    ...
    log.info("BYD brand_id=%s", brand_id)
