...
class TestLoad(unittest.TestCase):
    def setUp(self):
        ...
        self.cdll = patch("pyzbar.zbar_library.cdll", autospec=True).start()
        self.find_library = patch(
            "pyzbar.zbar_library.find_library", autospec=True
        ).start()
        self.platform = patch("pyzbar.zbar_library.platform", autospec=True).start()
        self.windows_fnames = patch(
            "pyzbar.zbar_library._windows_fnames",
            autospec=True,
    ...
    def test_found_windows(self):
        ...
        self.assertEqual(
            (self.cdll.LoadLibrary.return_value, [self.cdll.LoadLibrary.return_value]),
            res,
        )
    ...
    def test_found_second_attempt_windows(self):
        ...
        self.cdll.LoadLibrary.assert_has_calls(
            [
                call("dependency fname"),
                call(
                    str(Path(zbar_library.__file__).parent.joinpath("dependency fname"))
                ),
                call(str(Path(zbar_library.__file__).parent.joinpath("dll fname"))),
            ]
        )
...
class TestWindowsFnames(unittest.TestCase):
    ...
    def test_32bit(self):
        ...
        self.assertEqual(
            ("libzbar-32.dll", ["libiconv-2.dll"]), zbar_library._windows_fnames()
        )
    ...
    def test_64bit(self):
        ...
        self.assertEqual(
            ("libzbar-64.dll", ["libiconv.dll"]), zbar_library._windows_fnames()
        )
