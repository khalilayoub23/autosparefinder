...
export default function SocialLoginButtons({ redirectTo = '/', onSuccess }) {
  ...
  const handleSuccess = async (provider, token) => {
    try {
      ...
      const msg = (typeof detail === 'string' ? detail : detail?.message)
        || `שגיאה בכניסה עם ${provider === 'google' ? 'Google' : 'Facebook'}`
      toast.error(msg)
    } finally {
      setLoading(null)
    }
  ...
  const handleGoogle = () => {
    ...
    setLoading('google')
    try {
      // Use token client (popup flow) — more reliable than One Tap prompt()
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
          if (tokenResponse.error) {
            ...
            toast.error(
              `Google OAuth שגיאה: הוסף "${origin}" כ-Authorized JavaScript origin ב-https://console.cloud.google.com/apis/credentials`,
              { duration: 8000 }
            )
            setLoading(null)
            return
          ...
          try {
            ...
            // Send to backend as a special access-token flow
            await handleSuccess('google', tokenResponse.access_token)
          } catch {
      ...
      window.google.accounts.id.prompt((n) => {
        if (n.isNotDisplayed() || n.isSkippedMoment()) {
          const origin = window.location.origin
          toast.error(
            `Google: הוסף "${origin}" כ-Authorized JavaScript origin ב-https://console.cloud.google.com/apis/credentials`,
            { duration: 8000 }
          )
          setLoading(null)
        }
    ...
    window.FB.login(
      (resp) => {
        if (resp.authResponse) {
          ...
          // Domain not registered in the Facebook App settings
          const origin = window.location.hostname
          toast.error(
            `Facebook OAuth: הוסף "${origin}" ל-App Domains בهttps://developers.facebook.com/apps/${FACEBOOK_APP_ID}/settings/basic/`,
            { duration: 8000 }
          )
          setLoading(null)
        }
