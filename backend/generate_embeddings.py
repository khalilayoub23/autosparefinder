...
HF_TIMEOUT     = 10.0 # seconds per embedding call
...
async def embed_text(text_input: str) -> list[float] | None:
    """Call HF embedding API and return the vector, or None on failure."""
    try:
        return await hf_embed(text_input, timeout=HF_TIMEOUT)
    except Exception as exc:
        print(f"[EmbedGen] HF embedding error: {exc}")
        return None
...
async def run(dry_run: bool = False) -> None:
    if not os.getenv("HF_TOKEN", ""):
        print("[EmbedGen] ERROR: HF_TOKEN is not set in .env")
        sys.exit(1)
    ...
    print(
        ...
        f"(HF embedding model from hf_client)"
    )
    ...
    # Semaphore caps concurrent HF calls to CONCURRENT within each batch
    sem = asyncio.Semaphore(CONCURRENT)

    async def embed_limited(text_input: str) -> list[float] | None:
        async with sem:
            return await embed_text(text_input)
