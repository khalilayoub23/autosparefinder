...
from sqlalchemy import (
    Boolean, Column, DateTime, Float, ForeignKey, Integer, Numeric,
...
# ==============================================================================
# SOCIAL MEDIA CAMPAIGN TABLES (5)
# Added 2026-08-06: structured campaign management, group targeting, engagement
# tracking, analytics, and platform account management.
# ==============================================================================

class Campaign(Base):
    """A marketing campaign spanning one or more social platforms.

    Lifecycle: draft → active → paused | completed | archived.
    Linked to SocialPost rows via campaign_id in external_post_ids JSONB or
    directly via the campaigns_posts association kept in `linked_post_ids`.
    """
    ...
    goal = Column(Text, nullable=True, comment="Human description of campaign objective")
    platforms = Column(ARRAY(String), nullable=False, default=list)
    target_audience = Column(Text, nullable=True, comment="Who we're targeting (free text)")
    tone = Column(String(50), nullable=True, default="professional")
    duration_days = Column(Integer, nullable=True)
    budget_ils = Column(Float, nullable=True, comment="Approved budget in ILS")
    status = Column(String(20), nullable=False, default="draft")
    # The structured plan generated by SocialMediaManagerAgent.generate_campaign_plan()
    plan = Column(JSONB, nullable=False, default=dict)
    # UUIDs of social_posts linked to this campaign
    linked_post_ids = Column(ARRAY(String), nullable=False, default=list)
    # Aggregate performance (updated by feedback_analyzer.py)
    total_reach = Column(Integer, nullable=False, default=0)
    ...
    performance_score = Column(Float, nullable=True, comment="0-100 score computed by feedback analyzer")
...
class GroupTarget(Base):
    """A Facebook (or other) group that NOA may post into or monitor.

    Approval gate: `status='approved'` required before the browser agent
    may post. 'pending' rows are shown to the owner for review.
    """
    ...
    __table_args__ = (
        ...
        Index("ix_group_targets_platform_status", "platform", "status"),
    )

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    platform = Column(String(30), nullable=False, default="facebook",
                      comment="facebook | linkedin | reddit")
    group_name = Column(String(255), nullable=False)
    group_url = Column(String(500), nullable=False)
    group_id = Column(String(100), nullable=True, comment="Platform-native group ID if known")
    description = Column(Text, nullable=True)
    member_count_estimate = Column(Integer, nullable=True)
    relevance_tags = Column(ARRAY(String), nullable=False, default=list,
                            comment="e.g. ['toyota','brake-parts','israel']")
    status = Column(String(20), nullable=False, default="pending")
    # Aggregate stats updated by the browser agent and feedback loop
    posts_sent = Column(Integer, nullable=False, default=0)
    last_posted_at = Column(DateTime, nullable=True)
    avg_leads_per_post = Column(Float, nullable=True)
    # Approval metadata
    approved_by = Column(UUID(as_uuid=True), nullable=True)
    approved_at = Column(DateTime, nullable=True)
    rejection_reason = Column(Text, nullable=True)
...
class EngagementEvent(Base):
    """One engagement data-point for a published social post.

    Polled by feedback_analyzer.py from the platform APIs after publish.
    Multiple rows per post_id (one per polling cycle) — the latest is the
    freshest snapshot. Analyzer also writes lead/click events here.
    """
    __tablename__ = "engagement_events"
    __table_args__ = (
        Index("ix_engagement_events_post_id", "post_id"),
        Index("ix_engagement_events_platform_ts", "platform", "collected_at"),
        Index("ix_engagement_events_campaign_id", "campaign_id"),
    )

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    post_id = Column(UUID(as_uuid=True), nullable=False,
                     comment="social_posts.id — no FK (avoid cross-table lock during bulk inserts)")
    campaign_id = Column(UUID(as_uuid=True), nullable=True,
                         comment="campaigns.id — no FK (optional link)")
    platform = Column(String(30), nullable=False, comment="facebook|instagram|telegram|...")
    external_post_id = Column(String(200), nullable=True, comment="Platform native post ID")
    # Engagement counters (null = not yet polled or not available on this platform)
    likes = Column(Integer, nullable=True)
    comments = Column(Integer, nullable=True)
    shares = Column(Integer, nullable=True)
    reach = Column(Integer, nullable=True, comment="Unique accounts reached")
    impressions = Column(Integer, nullable=True, comment="Total times shown")
    clicks = Column(Integer, nullable=True, comment="Link clicks")
    saves = Column(Integer, nullable=True)
    leads = Column(Integer, nullable=True, comment="Confirmed lead conversions")
    # Sentiment (computed by feedback analyzer from sampled comments)
    sentiment_score = Column(Float, nullable=True, comment="-1.0 (negative) to 1.0 (positive)")
    sentiment_summary = Column(Text, nullable=True)
    # Source metadata
    data_source = Column(String(50), nullable=True,
                         comment="graph_api | manual | estimated")
    collected_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)


class PlatformAccount(Base):
    """Tracks credential status and health for each connected social platform.

    One row per platform. Updated by meta_client.py when tokens are refreshed
    or when API calls fail. The owner console reads this to show connection
    status without exposing raw secrets.
    """
    ...
    platform = Column(String(30), nullable=False, unique=True,
                      comment="facebook|instagram|whatsapp|telegram|tiktok|discord|reddit|youtube")
    status = Column(String(20), nullable=False, default="disconnected")
    account_name = Column(String(255), nullable=True, comment="Page/channel/account display name")
    account_id = Column(String(100), nullable=True, comment="Platform-native account ID")
    scopes = Column(ARRAY(String), nullable=False, default=list,
                    comment="OAuth scopes this token holds")
    token_expires_at = Column(DateTime, nullable=True, comment="NULL = non-expiring (page token)")
    ...
    capabilities = Column(JSONB, nullable=False, default=dict,
                          comment="e.g. {'can_post':true,'can_read_insights':true,'groups_api':false}")
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, nullable=False)


class AnalyticsReport(Base):
    """Periodic analytics snapshot computed by the feedback analyzer.

    One report per campaign per reporting period. Stored as structured JSONB
    so the Digital Department can review historical trends.
    """
    __tablename__ = "analytics_reports"
    __table_args__ = (
        Index("ix_analytics_reports_campaign_period", "campaign_id", "period_start"),
        Index("ix_analytics_reports_created", "created_at"),
    )

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    campaign_id = Column(UUID(as_uuid=True), nullable=True,
                         comment="campaigns.id — null for platform-wide reports")
    report_type = Column(String(50), nullable=False, default="campaign",
                         comment="campaign | platform | weekly | monthly")
    period_start = Column(DateTime, nullable=False)
    period_end = Column(DateTime, nullable=False)
    platform = Column(String(30), nullable=True, comment="null = all platforms")
    # Aggregated metrics
    ...
    total_clicks = Column(Integer, nullable=False, default=0)
    total_leads = Column(Integer, nullable=False, default=0)
    engagement_rate = Column(Float, nullable=True, comment="engagement/reach")
    click_through_rate = Column(Float, nullable=True)
    cost_per_lead_ils = Column(Float, nullable=True)
    # Best performers
    top_performing_post_id = Column(String(200), nullable=True)
    top_performing_platform = Column(String(30), nullable=True)
    top_performing_tone = Column(String(50), nullable=True)
    # Insights generated by the feedback analyzer LLM
    insights = Column(JSONB, nullable=False, default=dict,
                      comment="{'best_times':[], 'top_content_types':[], 'recommendations':[]}")
