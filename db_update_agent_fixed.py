...
# =========================================================================
# Task 8 – Extract spec prefix into specifications jsonb
# =========================================================================

def extract_spec_prefix(name: str) -> Tuple[str, Dict[str, str]]:
    """Extract leading spec tokens from a part name into a structured dict."""
    spec: Dict[str, str] = {}
    s = (name or "").strip()
    if not s:
        return s, spec

    # Pattern 1: Engine displacement, e.g. "2.4 חיישן גל זיזים"
    m = re.match(r'^(\d+\.\d+)\s+([א-ת])', s)
    if m:
        val = float(m.group(1))
        if 0.5 <= val <= 9.0:
            spec['engine'] = m.group(1)
            s = s[m.end(1):].strip()
            return s, spec

    # Pattern 2: Inch size, e.g. '15" מגב'
    m = re.match(r'^([\d\.]+)"\s+', s)
    if m:
        spec['size_inch'] = m.group(1)
        s = s[m.end():].strip()
        return s, spec

    # Pattern 3: Electrical rating, e.g. "20A נתיך", "25AMP פיוז"
    m = re.match(r'^(\d+(?:\.\d+)?)\s*[Aa][Mm]?[Pp]?\s+', s)
    if m and re.match(r'^\d+[Aa]', s):
        spec['rating'] = m.group(1) + 'A'
        s = s[m.end():].strip()
        return s, spec

    # Pattern 4: Wheel/rim size, e.g. "17X7 חישוק", "16X6.5 חישוק"
    m = re.match(r'^(\d+[xX][\d\.]+)\s+([א-ת])', s)
    if m:
    ...
    # Pattern 5: 4X4 followed by engine, e.g. "4X4 2.4 תיבת מהלכים"
    m = re.match(r'^4[xX]4\s+(\d+\.\d+)\s+([א-ת])', s)
    if m:
        spec['drivetrain'] = '4X4'
        spec['engine'] = m.group(1)
        s = s[m.end(1):].strip()
    ...
    # Pattern 6: Year prefix, e.g. "2021 רפידות בלם"
    m = re.match(r'^(20\d{2})\s+([א-ת])', s)
    if m:
        spec['year'] = m.group(1)
        s = s[m.end(1):].strip()
        return s, spec

    # Pattern 7: Wiper size, integer 8-36 before "מגב"
    m = re.match(r'^(\d{1,2})\s+(מגב)', s)
    if m:
        val = int(m.group(1))
        if 8 <= val <= 36:
            spec['size_inch'] = str(val)
            s = s[m.end(1):].strip()
            return s, spec

    # Pattern 8: MM size, e.g. "20 מוט מייצב" or "20 מ\"מ"
    m = re.match(r'^(\d{1,3})\s+(מוט\s+מייצב|מ"מ)', s)
    if m:
        spec['size_mm'] = m.group(1)
        s = s[m.end(1):].strip()
        return s, spec
...
async def extract_spec_prefixes(db: AsyncSession) -> Dict[str, Any]:
    """Extract spec prefixes from parts_catalog.name into specifications jsonb."""
    t0 = time.monotonic()
    BATCH_SIZE = 340
    ...
    try:
        rows = (await db.execute(text("""
            ...
            WHERE is_active = TRUE
              ...
              AND (
                  name ~ '^[0-9]+[.][0-9]+\\s+'
                  OR name ~ '^[0-9]+"\\s+'
                  OR name ~ '^[0-9]+[Aa][A-Za-z]*\\s+'
                  OR name ~ '^[0-9]+[xX][0-9]'
                  OR name ~ '^4[xX]4\\s+'
                  OR name ~ '^20[0-9]{2}\\s+'
                  OR name ~ '^[0-9]{1,2}\\s+מגב'
              )
            ORDER BY RANDOM()
            LIMIT :batch_size
        """), {"batch_size": BATCH_SIZE})).mappings().fetchall()

        if not rows:
            logger.info("[task8] No spec prefixes found - skipping")
            return {
                "task": "extract_spec_prefixes",
                "status": "ok",
                "scanned": 0,
                "updated": 0,
                "elapsed_s": round(time.monotonic() - t0, 2),
        ...
        updated = 0
        for row in rows:
            original_name = str(row["name"] or "")
            cleaned_name, new_spec = extract_spec_prefix(original_name)

            if not new_spec or cleaned_name == original_name:
                continue

            existing = row.get("specifications") or {}
            ...
            # Keep existing DB values when keys already exist.
            merged = {**new_spec, **existing}
            ...
            """), {
                "name": cleaned_name,
                "specifications": json.dumps(merged, ensure_ascii=False),
                "id": str(row["id"]),
            })

            logger.debug("[task8] '%s' -> name='%s' spec=%s", original_name, cleaned_name, new_spec)
            updated += 1

        await db.commit()
        logger.info("[task8] Extracted specs from %d/%d parts", updated, len(rows))
        return {
            "task": "extract_spec_prefixes",
            "status": "ok",
            "scanned": len(rows),
        ...
        await db.rollback()
        logger.error("extract_spec_prefixes failed: %s", exc)
        return {
            "task": "extract_spec_prefixes",
            "status": "error",
            "error": str(exc),
...
# =========================================================================
# Task 9 – Refresh min / max prices on parts_catalog
# =========================================================================
...
TASK_REGISTRY: Dict[str, Any] = {
    ...
    "extract_spec_prefixes":   extract_spec_prefixes,
